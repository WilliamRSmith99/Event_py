...
from commands.event import delete
...
class ManageEventButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        if not await shared.authenticate(interaction.user, self.event.organizer):
...
class ManageEventView(View):
    ...
    async def cancel_button(self, interaction: discord.Interaction, _):
        ...
        if await shared.authenticate(self.user, self.event.organizer):
    ...
    async def delete_button(self, interaction: discord.Interaction, _):
        ...
        await delete._prompt_event_deletion(
            interaction,
            self.guild_id,
            self.event.event_name,
            self.event_details,
            return_on_cancel=manage_event_context
        )
...
async def format_single_event(interaction, event, is_edit=False):
    ...
    if await shared.authenticate(interaction.user, event.organizer):
